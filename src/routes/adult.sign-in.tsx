...
export const Route = createFileRoute("/adult/sign-in")({ head: () => ({ meta: [{ title: "Parent sign in | RootBridge" }, { name: "description", content: "Sign in to securely sync family learning progress." }, { property: "og:title", content: "Parent sign in | RootBridge" }, { property: "og:description", content: "Secure family access and learning progress sync." }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary" }] }), component: ParentSignIn });
...
function ParentSignIn() {
  ...
  const emailAuth = async () => {
    ...
    unlockParentMode(); navigate({ to: "/adult/dashboard" });
  };
  const google = async () => { const result = await lovable.auth.signInWithOAuth("google", { redirect_uri: window.location.origin }); if (result.error) setMessage("Google sign-in could not start."); else if (!result.redirected) { unlockParentMode(); navigate({ to: "/adult/dashboard" }); } };
