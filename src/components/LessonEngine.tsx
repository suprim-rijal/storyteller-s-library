...
export function LessonEngine({ exercises, accent, romanization = true, quiet, reducedMotion, onFinish }: Props) {
  ...
  return <div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <Card accent={accent} bold className="flex flex-col items-center p-6"><div lang="ne" className={cn("grid w-full place-items-center rounded-2xl px-4 py-10 text-center text-5xl font-extrabold sm:text-6xl", accent === "language" ? "bg-language-soft text-language" : "bg-culture-soft text-culture")}>{(ex.kind === "hear-find" || ex.kind === "typing") && !audioNote ? "🔊" : ex.target.np}</div>{romanization ? <p className="mt-3 text-sm text-ink-soft">{ex.target.rom}</p> : null}<div className="mt-4 flex gap-2"><Button variant="grow" onClick={() => speak()}><AudioLines size={18} />Replay</Button><Button variant="outline" onClick={playing ? stop : () => speak(ex.instructions_audio, "en-US")}>{playing ? <Pause size={18} /> : <Play size={18} />}{playing ? "Pause" : "Instructions"}</Button></div><p aria-live="polite" className="mt-2 text-xs text-ink-soft">{playing ? "Audio is playing" : "Audio is paused"}</p>{audioNote ? <p className="mt-2 rounded-2xl bg-canvas p-3 text-xs text-ink-soft">{audioNote}</p> : null}</Card>
